...
//用户名
	...
	Name.onfocus = function () {
		...
		Name_msg.innerHTML = '请输入2~6个字的昵称(一个汉字为两个字符)';
	...
	Name.onblur = function () {
		...
		if (reName.test(this.value)) {
			Name_msg.innerHTML = '东哥是煞笔吗？含有非法字符，请重新输入';
			Name_msg.style.color = "red";

		}
		//不能为空
		else if (!this.value) {
			Name_msg.innerHTML = '东哥是煞笔吗？用户名不能为空';
			Name_msg.style.color = "red";
		}
		//长度超过12个
		else if (Name_length > 12) {
			Name_msg.innerHTML = '东哥是煞笔吗？用户名不能超过12个字符';
			Name_msg.style.color = "red";
		}
		//长度短于4个
		else if (Name_length < 4) {
			Name_msg.innerHTML = '东哥是煞笔吗？用户名不能少于4个字符';
			Name_msg.style.color = "red";
		}
		//OK
		else{
			Name_msg.innerHTML = '东哥是煞笔吗？昵称可用了';
...
//手机号码
	...
	PhoneNo.onfocus = function () {
		...
		PhoneNo_msg.innerHTML = '东哥请输入您的手机号码';
	...
	PhoneNo.onblur = function () {
		...
		if (this.value.length == 0) {
			PhoneNo_msg.innerHTML = '东哥是煞笔吗？手机号码不能为空';
			PhoneNo_msg.style.color = "red";
		}
		//要以1开头
		else if (!rePoneNo.test(this.value)) {
			PhoneNo_msg.innerHTML = '东哥是煞笔吗？请输入正确的手机号码';
			PhoneNo_msg.style.color = "red";
		}
		//要11位
		else if (this.value.length != 11) {
			PhoneNo_msg.innerHTML = '东哥是煞笔吗？手机号码位数不正确';
			PhoneNo_msg.style.color = "red";
		}
		//OK
		else{
			PhoneNo_msg.innerHTML = '东哥是煞笔吗？OK!';
...
//邮箱
	...
	EmailAdr.onfocus = function () {
		...
		EmailAdr_msg.innerHTML = '东哥请输入您的电子邮箱';
	...
	EmailAdr.onblur = function () {
		...
		//要完整
			...
			EmailAdr_msg.innerHTML = '东哥是煞笔吗？请输入正确的邮箱地址';
			EmailAdr_msg.style.color = "red";
		}
		//OK
		else{
			EmailAdr_msg.innerHTML = '东哥是煞笔吗？OK!';
...
//密码
	Passwd.onfocus = function () {
		...
		Passwd_msg.innerHTML = '请输入密码(6~20个字符)';
	...
	Passwd.onblur = function () {
		...
		//不能为空
		if (this.value.length == 0) {
			Passwd_msg.innerHTML = '东哥是煞笔吗？密码不能为空';
			Passwd_msg.style.color = "red";
		}
		//不能多于20个字符
		else if (this.value.length > 20) {
			Passwd_msg.innerHTML = '东哥是煞笔吗？密码不能多于20个字符';
			this.value = '';
			Passwd_msg.style.color = "red";
		}
		//不能少于6个字符
		else if (this.value.length < 6) {
			Passwd_msg.innerHTML = '东哥是煞笔吗？密码不能少于6个字符';
			this.value = '';
			Passwd_msg.style.color = "red";
		}
		//OK
		else{
			Passwd_msg.innerHTML = '东哥是煞笔吗？OK!';
...
//确认密码
	FrPasswd.onfocus = function () {
		...
		FrPasswd_msg.innerHTML = '东哥请再次输入密码';
	...
	FrPasswd.onblur = function () {
		...
		//两次密码不一样
		if (!this.value) {
			FrPasswd_msg.innerHTML = '东哥是煞笔吗？密码不能为空';
			this.value = '';
			FrPasswd_msg.style.color = "red";
		}
		else if (this.value != Passwd.value ) {
			FrPasswd_msg.innerHTML = '东哥是煞笔吗？两次密码不一样';
			this.value = '';
			FrPasswd_msg.style.color = "red";
		}
		//OK
		else{
			FrPasswd_msg.innerHTML = '东哥是煞笔吗？OK!';
